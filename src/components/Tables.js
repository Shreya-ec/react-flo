...
import { TbDeviceMobileMessage, TbEdit } from "react-icons/tb";
import Select from "react-select";
import { GrDocumentText } from "react-icons/gr";
import { FaCircleUser, FaDeleteLeft, FaRegEye } from "react-icons/fa6";
import {
  MdAppBlocking,
...
import { IoArrowBackCircle, IoDocumentTextOutline } from "react-icons/io5";
...
const ValueChange = ({ value, suffix }) => {
  ...
  return value ? (
    <span className={`d-flex align-items-center ${valueTxtColor}`}>
  ...
  const TableRow = (props) => {
    ...
    return (
      <tr className="border-bottom">
        ...
        <td>
          <span className="fw-bold">${parseFloat(price).toFixed(2)}</span>
        </td>
        <td>
        ...
        <td>
          <Row className="d-flex align-items-center">
            ...
            <Col xs={12} xl={10} className="px-0 px-xl-1">
              <ProgressBar
                variant="primary"
                className="progress-lg bg-dark mb-0"
                now={trafficShare}
                min={0}
                max={100}
              />
    ...
    <Card border="0" className="shadow mb-4">
      <Card.Body>
        <Table responsive className="table-centered table-nowrap rounded mb-0">
          ...
          <tbody className="border-0">
            {pageTraffic.map((pt, ind) => (
              <TableRow key={`page-traffic-${pt.id}`} index={ind + 1} {...pt} />
            ))}
          </tbody>
        </Table>
      </Card.Body>
